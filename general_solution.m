...
for i = t0:t_end+9
    ...
    t_end = i + 1;
    if i > 10
        disp("enter in RSU adjustment range .....");

        % if abs(sets_f(end).densities(1) - sets_f(end).densities(2)) < 0.000001 
        %     break;
        % end
        ...
        list_d(1) = list_d(1) + rand * 0.02;
        % list_d(end) = list_d(end) - d_shift_unit;
        list_d(end) = list_d(end) - rand * 0.05;
        if list_d(end) - list_d(1) < 0.0001 
            list_d(end) = list_d(1);
        end
        
        % build new speed-density function here

        ldfor = generate_ld_formula(list_x, list_d);
